"""Ingests the split pilot CSVs (Raw_data/split/) through the real Task 2
pipeline, in PRE-then-POST order per rail, and prints a summary.

Usage: .venv/Scripts/python.exe scripts/ingest_pilot_data.py
"""
    ...
    try:
        for rail_type in RAIL_TYPES:
            for stage in ("pre", "post"):
                fname = f"pilot_bank_{rail_type.lower()}_{stage}.csv"
