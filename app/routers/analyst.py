...
async def dashboard_priority_distribution(
    ...
    get_priority_distribution()'s docstring: sourced from the same real
    per-item severity_score/priority_level app/priority.py computes for
...
async def get_investigation_case(case_id: int, tenant_bank_id: str, db: Session = Depends(get_db)):
...
async def validate_investigation_case(case_id: int, body: ValidateCaseRequest, db: Session = Depends(get_db)):
    ...
    case = db.query(InvestigationCase).filter_by(id=case_id, tenant_bank_id=body.tenant_bank_id).one_or_none()
    if case is None:
        raise HTTPException(status_code=404, detail=f"No investigation case id={case_id} for this tenant")
